...
class GenericDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # self.visualize(ret['image'], ret['i_it_ctrs'])
        # self.visualize(ret['image'], ret['i_gt_ctrs'])
    ...
    def _load_pre_data(self, video_id, frame_id, sensor_id=1, slice_n=15):
        ...
        for i in Num:
            try:
                ...
                # 若找不到前后帧或标注，填充空数据
                imgs.append(np.zeros((self.default_resolution[0], self.default_resolution[1], 3), dtype=np.uint8))
                anns.append([])
                frame_dists.append(-1)
    ...
    def _get_pre_dets(self, anns, inp, trans_input, trans_output):
        ...
        trans = trans_input
        # 若anns或inp为空，直接返回空结构
        if not anns or not inp:
            return None, [], []
        if anns[0].get('image_id', 0) < inp[0].get('image_id', 0):
    ...
    def select_ct_tracking(self, pre_cts, ct_int, pre_anns, id, track_ids):
        candidate_tracks = []
        for ann_idx, ann in enumerate(pre_anns):
            if ann['category_id'] == id:
                if 'track_id' in ann:
                    current_track_id = ann['track_id']
                    for pre_ct_idx, tracked_id in enumerate(track_ids):
                        if tracked_id == current_track_id:
                            candidate_tracks.append(pre_cts[pre_ct_idx] - ct_int)
                            break

        track = None
        if len(candidate_tracks) != 0:
            if len(candidate_tracks) > 1:
                diff = [(t[0]**2 + t[1]**2)**0.5 for t in candidate_tracks]
                track = candidate_tracks[diff.index(min(diff))]
            else:
                track = candidate_tracks[0]
        return track
