...
def generate_contour(boxes, num=0):
    ...
    device = boxes.device
    ind = torch.linspace(0, num-1, num, device=device)
...
def test_cirpoint(dets, points, out_thresh):
    ...
    # i_it_ctrs = i_it_ctrs.cuda() # Removed hardcoded .cuda()
    # Assuming boxes is on the correct device, i_it_ctrs will be on boxes.device
